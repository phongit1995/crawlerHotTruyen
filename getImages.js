...
const  { isCloudflareJSChallenge ,makeId , isCloudflareJSV2Challenge} = require('./common');
const REFERER ="https://hotruyen.com/";
...
const getCookieCloudflare=async(proxy)=>{
    ...
    await page.goto("https://hotruyen.com/",{
        timeout:45000,
    ...
    response =>{
        return response.url() == 'https://hotruyen.com/' && response.status() == 200
    },{timeout:30000})
...
const getContentUrl= async (url="https://hotruyen.com/")=>{
    const cookie = await getCookieCloudflare();
    let options = {
        method:"GET",
        uri:url,
...
module.exports.getContentUrl = getContentUrl ;